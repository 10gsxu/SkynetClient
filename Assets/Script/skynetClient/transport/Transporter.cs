...
namespace Skynet.DotNetClient
{
    ...
    public class Transporter
    {
        ...
        private byte[] headBuffer = new byte[2];
        private byte[] buffer;
		...
		public void send(byte[] buffer, int length)
        {
            if (this.transportState != TransportState.closed)
            {
				...
				this.asyncSend = socket.BeginSend(buffer, 0, length, SocketFlags.None, new AsyncCallback(sendCallback), socket);
        ...
        private bool readHead(byte[] bytes, int offset, int limit)
        {
            ...
            if (length >= headNum)
            {
                ...
                //Get package length
				pkgLength = headBuffer[1] | (headBuffer[0] << 8);

                //Init message buffer
				buffer = new byte[pkgLength];
				offset += headNum;
				/*
        ...
        {
            int length = pkgLength - bufferOffset;
